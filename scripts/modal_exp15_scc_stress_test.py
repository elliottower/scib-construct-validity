"""Modal: Exp15 — SCC stress test (cross-classifier + bootstrap CIs).

Tests SCC robustness by computing it with 4 different classifiers:
  - Logistic regression (original, shares machinery with F1)
  - kNN (k=15)
  - Random forest (100 trees)
  - Linear SVM (Platt scaling)

F1 ground truth always uses logistic regression.
Runs on ALL qualifying tissues (targets 25, the full main panel).
Computes all 12 metrics + F1 per (tissue, model) condition.

After all tissues complete, computes:
  - Pooled Spearman rho with bootstrap 95% CIs (10,000 draws)
  - Within-tissue Kendall tau sign test
  - Pairwise model discrimination accuracy
  - Full 12-metric comparison table (9 original + 3 new SCC variants)

Usage:
    modal run --detach scripts/modal_exp15_scc_stress_test.py
    modal volume ls preflight-results exp15_scc_stress_test
    modal volume get preflight-results exp15_scc_stress_test results/exp15_scc_stress_test
"""
import modal

app = modal.App("preflight-exp15-scc-stress-test")
...
VOL_BASE = "/vol/exp15_scc_stress_test"
...
def compute_summary_stats(all_results):
    ...
    with open(summary_dir / "exp15_summary.json", "w") as f:
        json.dump(summary, f, indent=2, default=str)

    with open(summary_dir / "exp15_all_rows.json", "w") as f:
...
def orchestrate():
    ...
    with open(merged_dir / "exp15_all_tissues.json", "w") as f:
