"""Modal: Exp16b — 5-model panel with custom NMF baseline.

Same evaluation framework as exp15/16 (12 metrics, 4 SCC classifiers,
bootstrap CIs, BH correction) with 5 embedding models:
  - Geneformer (d=512, transformer) — from Census
  - scGPT (d=512, transformer) — from Census
  - scVI (d=50, VAE) — from Census
  - UCE (d=1280, transformer) — from Census
  - NMF (d=50, matrix factorization) — computed from raw counts

Census NMF embeddings have incomplete cell coverage (0% on Smart-seq2
target assay, partial on 10x source). We compute NMF ourselves via
sklearn on log1p-normalized counts, fitted jointly on source+target
per tissue. This gives full coverage and a true non-neural baseline.

Usage:
    modal run --detach scripts/modal_exp16b_with_nmf.py
    modal volume ls preflight-results exp16b_with_nmf
    modal volume get preflight-results exp16b_with_nmf results/exp16b_with_nmf
"""
import modal

app = modal.App("preflight-exp16b-with-nmf")
...
VOL_BASE = "/vol/exp16b_with_nmf"

CENSUS_VERSION = "2023-12-15"
ORGANISM = "homo_sapiens"
SOURCE_ASSAY = "EFO:0009922"
TARGET_ASSAY = "EFO:0008931"
CENSUS_EMBEDDINGS = ["geneformer", "scvi", "scgpt", "uce"]
ALL_EMBEDDINGS = ["geneformer", "scvi", "scgpt", "uce", "nmf_custom"]
NMF_COMPONENTS = 50
MAX_CELLS = 2000
MIN_SHARED_TYPES = 8
SEED = 20260801
...
def run_tissue(tissue_index, tissue_name, tissue_id):
    ...
    if vol_dir.exists() and result_path.exists():
        ...
        if all(f"metrics_{m}" in existing for m in ALL_EMBEDDINGS):
            if "scc_knn" in existing.get(f"metrics_{ALL_EMBEDDINGS[0]}", {}):
    ...
    print(f"  {_ts()} Computing custom NMF (d={NMF_COMPONENTS}) from raw counts...", flush=True)
    X_src_raw = src.X.toarray() if issparse(src.X) else np.asarray(src.X)
    X_tgt_raw = tgt.X.toarray() if issparse(tgt.X) else np.asarray(tgt.X)
    X_combined = np.vstack([X_src_raw, X_tgt_raw])
    X_combined = np.log1p(np.abs(X_combined))
    nmf_model = NMF(n_components=NMF_COMPONENTS, random_state=SEED, max_iter=500)
    nmf_embeddings = nmf_model.fit_transform(X_combined)
    src.obsm["nmf_custom"] = nmf_embeddings[:src.n_obs]
    tgt.obsm["nmf_custom"] = nmf_embeddings[src.n_obs:]
    print(f"  {_ts()} NMF done: reconstruction error={nmf_model.reconstruction_err_:.2f}", flush=True)
...
def compute_summary_stats(all_results):
    ...
    with open(summary_dir / "exp16b_summary.json", "w") as f:
        json.dump(summary, f, indent=2, default=str)

    with open(summary_dir / "exp16b_all_rows.json", "w") as f:
...
def orchestrate():
    ...
    with open(merged_dir / "exp16b_all_tissues.json", "w") as f:
