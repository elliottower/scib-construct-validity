...
and Procrustes (real mean 0.78 vs.\ null 0.96--0.99). An
analytical derivation shows the null floor follows
$\mathbb{E}[\mathrm{CKA}] \approx 1 - (k{-}1)/(d{+}k)$, matching
simulation within 0.02\%. A pre-registered spectral-gap statistic
designed to rescue discriminative validity fails: null baselines
exhibit \emph{higher} spectral concentration than trained models
(mean $z = 141$ vs.\ $88$; Mann--Whitney $p > 0.99$), confirming
that the null floor is structural rather than a correctable
normalization artifact. Bio-conservation metrics respond
non-monotonically to controlled embedding degradation (17--22 of 24
conditions). These results establish that predictive
...
Four findings emerge. First, most metrics predict transfer: Procrustes
...
biological signal. We derive an analytical formula showing that the null
floor follows $\mathbb{E}[\mathrm{CKA}] \approx 1 - (k{-}1)/(d{+}k)$,
explaining why the effect intensifies as $d/k$ grows. Third, a
pre-registered spectral-gap statistic designed to rescue
discriminative validity fails: null baselines have higher spectral
concentration than trained models, confirming that the null floor is
structural. Fourth, bio-conservation metrics respond non-monotonically
to controlled embedding degradation, with scores increasing at
intermediate noise levels before collapsing.
...
with $k \ll d$, the expected CKA admits a closed-form expression via
Wishart moments. Let $H = I_k - \frac{1}{k}\mathbf{1}\mathbf{1}^\top$
be the centering matrix. Using
$\mathbb{E}[\mathrm{tr}((HW H)^2)] = d^2(k{-}1)^2 + 2d(k{-}1)$ where
$W = XX^\top$ is Wishart, we obtain (Methods~\ref{sec:closedform}):
\begin{equation}
\label{eq:cka_null}
\mathbb{E}[\mathrm{CKA}(X, Y)] \approx 1 - c \cdot \frac{k-1}{d+k},
\quad c \approx 1.06
\end{equation}
This formula matches simulation within 0.02\% across $k = 8$--$44$ at
$d = 512$ (Table~\ref{tab:nullfloor}). The approximation explains why
the null floor tightens as $d/k$ increases: at $k = 8, d = 512$,
$\mathbb{E}[\mathrm{CKA}] = 0.985$; at $k = 44, d = 512$,
$\mathbb{E}[\mathrm{CKA}] = 0.921$. Embeddings that preserve biological
structure introduce correlations among cell-type centroids, reducing CKA
...
The null floor has an analytical explanation. For random Gaussian
centroid matrices, $\mathbb{E}[\mathrm{CKA}] \approx 1 - (k{-}1)/
(d{+}k)$, matching simulation within 0.02\%. At $d = 512$ and
$k = 8$--$44$, this yields null CKA of 0.92--0.99. The formula
follows from Wishart moments of the centered Gram matrix and reduces
to a ratio of the number of centroid degrees of freedom ($k - 1$
after centering) to the total parameter count ($d + k$). As $d/k$
grows, random centroid matrices become more similar because each
centroid occupies a larger fraction of an effectively empty space.

The pre-registered spectral-gap experiment confirms that this null
floor is structural. The spectral-gap ratio was designed to exploit
eigenvalue structure---a property CKA and Procrustes do not
measure---to distinguish trained from random embeddings. Trained
models show \emph{less} spectral concentration than random
projections, the opposite of the pre-registered prediction. The
reversal arises because trained models spread cell types across
embedding dimensions to improve separability, while random projections
preserve the input's dominant variance axis. Any projection of
biologically structured gene expression produces concentrated centroid
spectra; the concentration reflects input structure, not learned
representations.
...
validation to two of three datasets. The spectral-gap test is one of
many possible statistics; other approaches (mutual information between
centroid assignments, topological features) might rescue discriminative
validity. The closed-form CKA null is exact for Gaussian inputs; real
centroid distributions are non-Gaussian, though the simulation match
suggests the approximation is tight in practice. We release all scripts,
