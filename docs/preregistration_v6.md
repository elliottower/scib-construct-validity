# Preflight Bio — Experiment Preregistration (v6)

**Status:** FROZEN — SHA computed after scorer bug fixes, before re-run  
**Date:** 2026-07-11  
**Author:** Elliot Tower  
**Frozen SHA (exp0):** `2561c5d5cc4e1a144e059bf295b414fe4e5fda343842f2c83d9ea02f49d4d470`  
**Git commit:** `473dad622cafb8f8b166743327b40946c53b0082`  
**Frozen artifacts:** `docs/frozen_prereg_v6/`

**Changes from v5:** Three scorer implementation bug fixes diagnosed from v5 Exp0 results (10 valid pairs). No hypotheses, endpoints, thresholds, or weights changed. Module weights remain: M1=2.0, M2=2.0, M3=1.0, M4=1.0, M5=1.5, M6=0.5.

1. **M4: participation ratio replaces NaN counter.** M4 (domain validity) counted NaN values to measure embedding quality. On Census geneformer embeddings, which contain no NaNs, this produced score=1.0 for all pairs regardless of actual dimensional structure. Fixed: M4 now computes the participation ratio `(sum(lambda))^2 / sum(lambda^2) / d` for each domain, scoring the ratio min(PR_s, PR_t)/max(PR_s, PR_t). This measures effective dimensionality similarity.

2. **M6: polarity corrected.** M6 (curvature) computes the fraction of bottleneck edges in the cell-type KNN graph. The score was `1 - frac_bottleneck`, meaning graphs with more focused connectivity scored lower. Fixed: score = `frac_bottleneck` directly. More bridge edges = more structured connectivity = higher score.

3. **M3: LDA subspaces when labels provided.** M3 (direction stability) used PCA subspaces, making it identical to M1 (Grassmannian alignment) — v5 showed M1-M3 correlation of 0.994. Fixed: when cell-type labels are available, M3 fits LDA subspaces via `fit_lda_subspace()`, measuring discriminant direction stability rather than variance direction stability. Falls back to PCA when labels are absent.

**Transparency note:** All three fixes are implementation bugs, not metric redesigns. The v5 results (M4=1.0 for all pairs, M1-M3 rank correlation 0.994) are reported as evidence of the bugs. The v6 re-run tests the same hypotheses with the corrected scorer. Both v5 and v6 results are reported side-by-side.

**Secondary analysis (predeclared):** M6 sensitivity sweep. M6 scores are uniformly low (0.10-0.23) across all pairs because cell-type KNN graphs have inherently few bottleneck edges. This is a calibration finding, not a bug. Composites will be recomputed with M6 weight in {0.0, 0.1, 0.25, 0.5} to assess whether the composite is stable under reasonable M6 reweighting. This sweep is a robustness check, not a post-hoc optimization.

**Prior versions:** v5 (M7/gate/threshold), v4 (original), v3 (H0.2/H0.3 bin-size gate), v2 (Perplexity review fixes)
