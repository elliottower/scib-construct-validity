...
Computable Failure Boundaries Across Twelve Metrics}
...
model captures biological signal. We show these come apart. CKA and
Procrustes on cell-type centroids rank embeddings by downstream transfer
(partial $\rho = +0.53$ and $+0.66$), but their expected value under a
random Gaussian null follows $\mathbb{E}[\mathrm{CKA}] \approx 1 -
1.06\,(k{-}1)/(d{+}k)$, producing null saturation at foundation-model
dimensionality ($d \geq 512$): the analytic null exceeds all observed scores.
The boundary falls inside the dimensionality range the field uses: at
$d = 50$, 86\% of conditions clear the null; at $d \geq 512$, none.
Dimensionality reduction accounts for the large majority of this effect
(random projections to $d = 50$ clear the null in 46--48\% of
conditions), with a measurable denoising component from PCA (59\%).
scGraph metrics, operating on $k \times k$ cell-type relationship
matrices, provide a pre-registered confirmatory alternative that is
dimensionality-stable and discriminates embedding quality at high $d$.
We propose a usability criterion: given $k$ shared cell types and
dimension $d$, compute $\mathbb{E}[\mathrm{CKA}]$; if scores do not
exceed it, CKA is uninformative as a certification tool. Every current
foundation model violates this criterion.
\end{abstract}

\noindent\textbf{Keywords:} single-cell foundation models, embedding evaluation, construct validity, centered kernel alignment, Procrustes analysis, scIB, scGraph
...
Six findings emerge. First, most metrics rank well: Procrustes
...
vs.\ random projections 46--48\%). Third, scGraph metrics, which operate
on $k \times k$ cell-type relationship matrices, are
dimensionality-stable and discriminate embedding quality in a
pre-registered confirmatory test. Fourth, two attempted repairs fail in
diagnosable ways: a pre-registered spectral-gap statistic inverts (null
baselines more concentrated than trained models), and $z$-score
normalization degrades ranking by removing real $k$-dependent signal.
Fifth, multi-seed noise calibration (10 seeds, 22 tissues) shows that
apparent non-monotonicity in Leiden-dependent bio-conservation metrics is
clustering jitter: ARI drops from 70\% to 6.7\% non-monotonic after
permutation calibration (10.5$\times$ inflation). Sixth, a cross-tissue
...
\subsection{scGraph: a dimensionality-independent alternative}
\label{sec:scgraph}
...
scGraph metrics \citep{wang2026scgraph} evaluate relational structure
between cell types via $k \times k$ relationship matrices---centroid
distance correlations, $k$NN overlap, and weighted affinity---then
correlate the upper-triangular entries across assays. Because these
metrics operate on $k \times k$ matrices, they should be invariant to
CKA null saturation by construction.
...
three scGraph variants were evaluated at native $d$ and at PCA-reduced
$d = 50$ for the $d = 512$ models.

\begin{table}[t]
\centering
\caption{Pre-registered scGraph predictions and outcomes. Primary
...
\midrule
P-3 & $\rho(\text{scGraph}, \text{F1}) \geq 0.30$ & $\geq 0.30$ & FAIL (0.24) \\
P-4 & CKA $<$ null $\geq 90\%$; scGraph $\geq 0.20$ $\geq 80\%$ & 90\%/80\% & \textbf{PASS} (100\%/93\%) \\
P-5 & scVI ranking tracks F1, $\geq 60\%$ & $\geq 60\%$ & FAIL (50\%) \\
\bottomrule
\end{tabular}
\end{table}

The primary decision rule is satisfied: scGraph is dimensionality-stable
...
range. Geneformer produces higher scGraph scores than scGPT in 17 of
22 tissues (77.3\%), matching the transfer-F1 ordering.

The dissociation is complete: 100\% of $d = 512$ conditions fall below
the CKA analytic null, yet 93\% produce scGraph centroid distance
correlations $\geq 0.20$.

The secondary predictions provide boundaries on what scGraph measures.
The pooled Spearman correlation between scGraph and transfer F1
($\rho = 0.24$, $p = 0.054$) falls short of the 0.30 threshold (P-3).
The correlation is driven by between-model variance---Geneformer has
both higher F1 and higher scGraph than scGPT. scGraph tracks which
model produces better relational structure across assays; tissue-level
variation within a model falls outside its resolution. The scVI ranking
prediction (P-5) fails at 50\% agreement, likely because scVI's lower
dimensionality ($d = 128$) inflates $k$NN-based correlations
independently of embedding quality.
...
scGraph metrics provide a constructive alternative. scGraph operates on
$k \times k$ cell-type relationship matrices, bypassing the
$d$-dimensional embedding space. It is dimensionality-stable (median
change of 0.017 between native $d = 512$ and PCA-reduced $d = 50$)
and discriminates embedding quality (Geneformer outscores scGPT in
77\% of tissues, matching transfer-F1 ordering). The dissociation is
complete: 100\% of $d = 512$ conditions fall below the CKA null, yet
93\% produce scGraph centroid distances $\geq 0.20$. scGraph's pooled
correlation with F1 is modest ($\rho = 0.24$), driven by between-model
variance; within-model tissue-level prediction is absent. The metric
measures relational structure preservation across assays, complementing
CKA at low $d$ and filling the gap at high $d$.
...
transfer prediction when cell-type labels are available. Sixth, consider
scGraph metrics for cross-assay evaluation at $d > 200$; these metrics
operate on cell-type relationship matrices and are invariant to the
dimensionality-dependent CKA null saturation.
...
modulates the effect size within the certifiable regime. The noise dose-response experiment uses Gaussian additive noise;
extending to other degradation types (dropout, adversarial perturbation)
would test robustness beyond the Gaussian case. scGraph's modest
within-model tissue-level prediction ($\rho = 0.24$) suggests it
captures relational structure preservation; whether relational structure
is sufficient for practical model selection in
dimensionality-heterogeneous panels remains open. The
...
\subsection{scGraph metrics}
\label{sec:scgraphmethod}

Three relational-structure metrics were computed per (tissue, model)
condition. For $k$ shared cell types, compute $k \times k$ matrices
on both source and target embeddings: (1) centroid Euclidean distance
matrix; (2) $k$NN overlap matrix (fraction of cross-type $k = 15$
nearest-neighbor edges between each cell-type pair); (3) weighted
affinity matrix (mean inverse distance). The metric is the Spearman
correlation between the upper-triangular entries of the source and
target matrices. All three metrics were computed at native $d$ and at
PCA-reduced $d = 50$ for models with $d > 50$.
...
\section{scGraph pre-registration}
\label{app:scgraph}
...
$\Rightarrow$ scGraph is also dimensionality-affected; investigate
mechanism.
...
100\% below null; scGraph: 93\% $\geq 0.20$). P-5 FAIL (50\%
agreement).
