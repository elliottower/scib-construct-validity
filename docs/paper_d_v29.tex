...
\title{A Construct Validity Crisis in Single-Cell Embedding Evaluation:\\
Failure Boundaries and Source Classifier Confidence}
...
\begin{abstract}
Single-cell embedding metrics serve two roles the field conflates:
\emph{ranking} models against each other, and \emph{certifying} that a
model captures biological signal. We show these come apart. CKA on
cell-type centroids ranks embeddings by downstream transfer (partial
$\rho = +0.53$), but its expected value under a random Gaussian null
follows $\mathbb{E}[\mathrm{CKA}] \approx 1 -
1.06\,(k{-}1)/(d{+}k)$, producing null saturation at foundation-model
dimensionality ($d \geq 512$): the analytic null exceeds all observed
scores. Relational consistency scores (RCS), operating on $k \times k$
cell-type relationship matrices, are dimensionality-stable but achieve
only marginal transfer prediction ($\rho = 0.25$, pairwise accuracy
68\%). We tested transfer-aware alternatives under a registered
evaluation framework (SHA-256 frozen before computation; metric design
was informed by exploratory analysis of RCS). Source classifier
confidence (SCC)---the mean maximum predicted probability of a
source-trained cell-type classifier applied to target cells---achieves
$\rho = 0.77$ with transfer F1 ($p < 10^{-13}$, BH-corrected across
twelve metrics), 83\% pairwise model discrimination, and concordant
Kendall $\tau$ in 23 of 23 tissues. A pre-registered cross-classifier
stress test confirms SCC's predictive power survives when the SCC
classifier (kNN, random forest, or linear SVM) differs from the F1
classifier (logistic regression), ruling out a shared-machinery
artifact. MMD and class-conditional alignment (here CCAL; distinct
from canonical correlation analysis) also pass BH correction. The
failure of existing metrics and success of SCC share a common
explanation: relational metrics measure geometric similarity but are
invariant to cluster separability, while SCC directly measures whether
learned decision boundaries transfer.
\end{abstract}

\noindent\textbf{Keywords:} single-cell foundation models, embedding evaluation, construct validity, centered kernel alignment, source classifier confidence, transfer prediction, scGraph
...
data---achieves $\rho = 0.77$ with transfer F1 ($p < 10^{-13}$,
BH-corrected across twelve metrics), 83\% pairwise model discrimination,
and concordant Kendall $\tau$ in 23 of 23 tissues; a cross-classifier
stress test (kNN, random forest, SVM) confirms this is an
embedding-space property, not a classifier artifact. Fifth, two attempted
repairs fail in diagnosable ways: a pre-registered spectral-gap
statistic inverts (null baselines more concentrated than trained models),
...
strongest single statistic ($\rho = 0.77$, unanimous tissue-level
concordance across 23 tissues).
...
target dimension. None clears the pre-registered 60\% threshold.
...
No projection method clears the pre-registered 60\% threshold
...
Five predictions were pre-registered before data access (SHA-256
frozen; Supplementary~\ref{app:scgraph}). All 22 tissues, three
models (Geneformer $d = 512$, scGPT $d = 512$, scVI $d = 128$), and
three RCS variants were evaluated at native $d$ and at PCA-reduced
$d = 50$ for the $d = 512$ models.

\begin{table}[t]
\centering
\caption{Pre-registered RCS predictions and outcomes. Primary
predictions (P-1, P-2) determine the decision rule; both pass.
Secondary predictions (P-3--P-5) provide additional characterization.}
\label{tab:scgraph}
\small
\begin{tabular}{@{}lp{5.5cm}ll@{}}
\toprule
ID & Prediction & Threshold & Outcome \\
\midrule
P-1 & Median $|\Delta|$ across $d$ & $< 0.10$ & \textbf{PASS} (0.017) \\
P-2 & Geneformer $>$ scGPT in $\geq 70\%$ tissues & $\geq 70\%$ & \textbf{PASS} (77.3\%) \\
\midrule
P-3 & $\rho(\text{RCS}, \text{F1}) \geq 0.30$ & $\geq 0.30$ & FAIL (0.24) \\
P-4 & CKA $<$ null $\geq 90\%$; RCS $\geq 0.20$ $\geq 80\%$ & 90\%/80\% & \textbf{PASS} (100\%/93\%) \\
P-5 & scVI ranking tracks F1, $\geq 60\%$ & $\geq 60\%$ & FAIL (50\%) \\
\bottomrule
\end{tabular}
\end{table}

The primary decision rule is satisfied: RCS is dimensionality-stable
and discriminates embedding quality (Table~\ref{tab:scgraph}). The
centroid distance correlation changes by a median of only 0.017
between native $d = 512$ and PCA-reduced $d = 50$ (IQR: 0.009--0.047),
compared to CKA's analytic null shift from 0.98 to 0.84 over the same
range. Geneformer produces higher RCS scores than scGPT in 17 of
...
The secondary predictions reveal RCS's limitation. The pooled Spearman
correlation between RCS and transfer F1 ($\rho = 0.25$, $p = 0.054$)
falls short of the 0.30 threshold (P-3). The correlation is driven by
between-model variance---Geneformer has both higher F1 and higher RCS
than scGPT. RCS tracks which model produces better relational structure
across assays; tissue-level variation within a model falls outside its
resolution. The failure is structural: Spearman correlation on centroid
distances is invariant to monotonic scaling, so an embedding space where
clusters overlap (low F1) scores identically to one where clusters are
well-separated (high F1), as long as relative distances are preserved.
This motivates a search for metrics that capture cluster separability
directly.
...
separability and conditional alignment. We specified candidates in
a registered analysis plan (SHA-256:
\texttt{f5d472f5\ldots dd992}; metric design was informed by
understanding the RCS failure mode; the evaluation framework and
multiple-comparison correction were frozen before computation). A
follow-up cross-classifier stress test (SHA-256:
\texttt{727ec262\ldots e899}) tested four classifier families for SCC.
...
\begin{table}[t]
\centering
\caption{Transfer-aware metric comparison across 23 tissues
$\times$ 3 models (69 conditions). Spearman $\rho$ with transfer F1
(bootstrap 95\% CI from 10{,}000 resamples),
pairwise model discrimination accuracy (binomial test), and
within-tissue Kendall $\tau$ sign test (primary criterion). All
$p$-values BH-corrected across twelve metrics. SCC variants use
four classifier families; F1 ground truth always uses logistic
regression.}
\label{tab:scc}
\small
...
\midrule
SCC (LR)         & $+0.768$ & $[+.64,\;+.85]$ & $<0.001$ & 82.6\% & 23/23 & $<0.001$ \\
SCC (kNN)        & $+0.507$ & $[+.31,\;+.66]$ & $<0.001$ & 78.3\% & 20/23 & $<0.001$ \\
SCC (RF)         & $+0.587$ & $[+.38,\;+.74]$ & $<0.001$ & 79.7\% & 21/23 & $<0.001$ \\
SCC (SVM)        & $+0.517$ & $[+.30,\;+.69]$ & $<0.001$ & 72.5\% & 20/23 & $<0.001$ \\
\midrule
MMD              & $+0.595$ & $[+.42,\;+.73]$ & $<0.001$ & 87.0\% & 23/23 & $<0.001$ \\
CCAL             & $+0.598$ & $[+.43,\;+.72]$ & $<0.001$ & 89.9\% & 23/23 & $<0.001$ \\
\midrule
RCS (baseline)   & $+0.298$ & $[+.07,\;+.50]$ & $0.019$  & 72.5\% & 21/23 & $<0.001$ \\
RCS (PCA-10)     & $+0.269$ & $[+.02,\;+.49]$ & $0.028$  & 60.9\% & 16/23 & $0.051$ \\
RCS (trimmed)    & $+0.293$ & $[+.07,\;+.49]$ & $0.019$  & 72.5\% & 21/23 & $<0.001$ \\
RCS (normalized) & $+0.286$ & $[+.06,\;+.49]$ & $0.021$  & 71.0\% & 19/23 & $0.002$ \\
RCS (combined)   & $+0.316$ & $[+.07,\;+.52]$ & $0.014$  & 65.2\% & 18/23 & $0.006$ \\
\midrule
PAD              & $+0.112$ & $[-.13,\;+.34]$ & $0.359$  & 51.6\% & 10/23 & $0.798$ \\
\bottomrule
\end{tabular}
\end{table}

SCC with logistic regression achieves the strongest pooled correlation
with F1 ($\rho = +0.77$, $p < 10^{-13}$, BH-corrected across twelve
metrics), and the within-tissue Kendall $\tau$ is positive in all 23
tissues (Table~\ref{tab:scc}). Replacing the SCC classifier with
kNN ($\rho = +0.51$), random forest ($\rho = +0.59$), or linear SVM
($\rho = +0.52$) preserves the relationship, ruling out a shared-classifier
artifact: all four SCC variants pass BH correction on every test.
MMD and CCAL produce concordant results ($\rho = +0.60$ each, 23/23
concordant), providing convergent evidence from three theoretical angles.

PAD---the metric most directly grounded in domain-adaptation
theory---fails entirely ($\rho = +0.11$, non-significant). PAD
...
produce $\rho \in [0.21, 0.30]$, confirming that the RCS failure is
structural rather than a correctable implementation detail.


\subsection{Null saturation is structural: pre-registered spectral-gap test}
\label{sec:spectral}
...
the metric over baseline across noise levels. $H > 0$ indicates non-monotonicity:
the metric \emph{increased} at some noise level. Raw positivity
counts conditions where $H$ exceeds 0.01 averaged across seeds.
Calibrated positivity applies a permutation null (1{,}000
...
The calibration resolves the noise-sensitivity question. Leiden-dependent
metrics report improvement when an embedding is degraded, but this is
stochastic---driven by clustering jitter. Metrics built from
deterministic computations (silhouette, ASW) respond reliably; metrics
mediated by Leiden clustering respond spuriously.
...
source embeddings transfer---the property that F1 itself
quantifies. The correlation is $\rho = 0.77$ ($p < 10^{-13}$), and
the concordance is unanimous (23/23 tissues). A pre-registered
cross-classifier stress test confirms that SCC's predictive power
survives when the SCC classifier (kNN, random forest, linear SVM)
differs from the F1 classifier (logistic regression), with all four
classifier families passing BH correction ($\rho = 0.51$--$0.77$).
This rules out the concern that SCC-F1 correlation merely reflects
shared logistic-regression capacity. MMD and CCAL converge on the same
conclusion from distributional-divergence and centroid-alignment
perspectives ($\rho \approx 0.60$ each, 23/23 concordant). The pattern
is clear: metrics that measure properties relevant to classification
...
The pre-registered spectral-gap experiment confirms that null saturation
...
ranking/certification dissociation. Under controlled degradation (10
seeds per condition, 22 tissues), Leiden-dependent bio-conservation
metrics show massive inflation between raw and calibrated
non-monotonicity: ARI drops from 70\% to 6.7\% non-monotonic
(10.5$\times$ inflation), graph connectivity from 48\% to 2.5\%
(19.3$\times$). Most apparent metric increases under noise reflect
clustering jitter: permutation calibration eliminates the effect.
Deterministic metrics (silhouette, isolated-label ASW) show minimal
inflation (1.0--1.1$\times$).
...
hyperparameters beyond the classifier itself, achieves $\rho = 0.77$
with transfer F1, and is computable from the same data any
cross-technology evaluation already uses. The correlation survives
when the SCC classifier differs from the F1 classifier ($\rho = 0.51$
...
increase statistical power. The PCA-reduction control confirms the
causal role of dimensionality for Geneformer (18/22 tissues clear the
null at $d = 50$), though scGPT's lower recovery rate (8/22) suggests
that model architecture modulates the effect size within the certifiable
regime. SCC's strong correlation with F1 survives replacement of the
SCC classifier with kNN ($\rho = 0.51$), random forest ($\rho = 0.59$),
or linear SVM ($\rho = 0.52$), ruling out shared logistic-regression
capacity as the primary driver. SCC does not detect
label-swap pathologies: an embedding that maps cell types to the wrong
regions still produces confident predictions, so SCC would score high
despite low F1. SCC and CCAL require cell-type labels in both source and
target; in deployment settings where target labels are unavailable, only
PAD and MMD apply. The within-tissue Kendall $\tau$ test has only
$n = 3$ models per tissue, making individual tissue-level $\tau$ values
coarse ($\tau \in \{-1, -1/3, +1/3, +1\}$); the pooled Spearman
($n = 69$) is the more informative statistic. The transfer-aware metrics
analysis plan was registered after exploratory analysis of RCS and
official scGraph, so metric design was informed by data; the evaluation
framework (BH correction, primary success criterion) was frozen before
computation. The noise dose-response experiment uses Gaussian additive
noise; extending to other degradation types (dropout, adversarial
perturbation) would test robustness beyond the Gaussian case. The
...
inversion-rate estimates. The PBMC ceiling effect limits external
validation to two of three datasets. The closed-form CKA null is exact
for Gaussian inputs; real centroid distributions are non-Gaussian, though
the simulation match suggests the approximation is tight in practice. We
...
across 23 tissues and three models (Geneformer, scGPT, scVI), yielding
69 conditions. Ground truth is macro-averaged logistic-regression F1 on
...
access. The RCS and spectral-gap experiments were pre-registered before
any data access. The transfer-aware metrics analysis plan (SHA-256:
\texttt{f5d472f5\ldots dd992}) was registered after exploratory analysis
of RCS and official scGraph had been completed; metric design was
informed by understanding the RCS failure mode, but the evaluation
framework (BH correction across nine metrics, primary success criterion
on within-tissue Kendall $\tau$ sign test) was frozen before any
transfer-aware metric computation. A follow-up cross-classifier stress
test (SHA-256: \texttt{727ec262\ldots e899}, amended from v1) extends
SCC to four classifier families (kNN, random forest, linear SVM,
logistic regression) under BH correction across 12 metrics. SHA-256
hashes and the full deviation log are in the online repository.
...
\section{Noise dose-response: single-seed trajectories}
\label{app:noise}
...
multi-seed calibration eliminates (Section~\ref{sec:noise}). NMI in
brain/Geneformer shows scores \emph{increasing} at high noise
($\sigma \geq 1.0$): noise drives all cells into fewer Leiden clusters,
mechanically raising NMI. These per-seed trajectory changes are real but
do not exceed what random permutation of noise labels produces.
...
\section{RCS pre-registration}
\label{app:scgraph}

Five predictions were pre-registered with explicit thresholds and
decision rules before data access. The pre-registration document,
SHA-256 hash, and full results are in the online repository.

\paragraph{Predictions.} P-1 (primary, dimensionality-stable): median
$|\Delta\text{RCS}|$ between native $d$ and PCA $d = 50$ is $< 0.10$
for each variant. P-2 (primary, discriminates quality): Geneformer
exceeds scGPT on centroid distance correlation in $\geq 70\%$ of
tissues. P-3 (secondary): Spearman $\rho$(RCS, F1) $\geq 0.30$
across all conditions. P-4 (secondary, dissociation): CKA below null
in $\geq 90\%$ of $d = 512$ conditions and RCS $\geq 0.20$ in
$\geq 80\%$. P-5 (secondary): scVI ranking tracks F1 with $\geq 60\%$
agreement.

\paragraph{Decision rule.} P-1 and P-2 both pass $\Rightarrow$ RCS
is a dimensionality-independent alternative. P-1 pass, P-2 fail
$\Rightarrow$ stable but not discriminative; negative result. P-1 fail
$\Rightarrow$ RCS is also dimensionality-affected; investigate
mechanism.

\paragraph{Results.} P-1 PASS (median $\Delta$: centroid distance
0.017, $k$NN overlap 0.046, weighted affinity 0.052). P-2 PASS (17/22
= 77.3\%). P-3 FAIL ($\rho = 0.24$, $p = 0.054$). P-4 PASS (CKA:
100\% below null; RCS: 93\% $\geq 0.20$). P-5 FAIL (50\%
agreement).


\section{Transfer-aware metrics: registered analysis plan}
\label{app:scc}

The analysis plan for transfer-aware metrics was registered after
exploratory analysis of RCS and official scGraph. Metric design was
informed by understanding the RCS failure mode (invariance to
cluster separability). The evaluation framework and multiple-comparison
correction were frozen before computation (SHA-256:
\texttt{f5d472f56458e8792e56903386bfde830400f143ffd54ccdd5def1af578dd992}).

\paragraph{Metrics tested.} Five RCS ablations (baseline, PCA-10,
trimmed-mean centroids, column-max normalization, combined), four SCC
classifier variants (logistic regression, kNN, random forest, linear
SVM), and three additional metrics (MMD, CCAL, PAD). Definitions in
Section~\ref{sec:sccmethod}.

\paragraph{Evaluation framework.} Three tests per metric: (1) pooled
Spearman $\rho$ with transfer F1 ($n = 69$); (2) pairwise model
discrimination accuracy with binomial test; (3) within-tissue Kendall
$\tau$ sign test ($n = 23$ independent observations). BH correction
across all twelve metrics, applied separately to each test type. Primary
success criterion: BH-corrected $p < 0.05$ on the within-tissue Kendall
$\tau$ sign test.

\paragraph{Results.} Six metrics pass the primary criterion
decisively: all four SCC variants (20--23/23 positive $\tau$,
BH $p < 0.001$), MMD (23/23, BH $p < 0.001$), and CCAL (23/23,
BH $p < 0.001$). Four RCS variants pass: RCS-baseline (21/23,
BH $p < 0.001$), RCS-trimmed (21/23, BH $p < 0.001$),
RCS-normalized (19/23, BH $p = 0.002$), and RCS-combined (18/23,
BH $p = 0.006$). RCS-PCA10 is marginal (16/23, BH $p = 0.051$).
PAD fails (10/23, BH $p = 0.798$). Full results in
Table~\ref{tab:scc}.

\paragraph{What this plan does not pre-register.} Which metric will win
(strong prior on SCC from exploratory analysis), effect size thresholds
for ``good enough,'' or whether to pursue any metric further based on
results.
