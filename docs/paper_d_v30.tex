...
applied to Smart-seq2) across 23 human tissues and 69 embedding
...
controlling for dimensionality) yet exhibits \emph{null saturation}:
...
transfer, achieving Spearman $\rho = 0.77$ ($p < 10^{-13}$,
Benjamini-Hochberg-corrected across twelve metrics). A pre-registered
cross-classifier stress test confirms this result is robust across
classifier families (kNN, random forest, linear SVM), ruling out
shared-machinery confounding with the logistic-regression F1
classifier.
...
alternative, robust across classifier families, for predicting
cross-assay transfer. Benchmark rankings derived from relational
...
\subsection{Null saturation is structural: spectral-gap test}
\label{sec:spectral}
...
the concordance is unanimous (23/23 tissues). A cross-classifier
stress test confirms that SCC's predictive power
...
The spectral-gap experiment confirms that null saturation is structural.
The spectral-gap ratio was designed to exploit eigenvalue structure to
distinguish trained from random embeddings. Trained models show
\emph{less} spectral concentration than random projections. The
reversal arises because
...
with transfer F1, requires labels only in the source domain, and is
computable from the same data any cross-technology evaluation already
uses. SCC can be added to existing benchmark suites (e.g., scIB) as a
...
capacity as the primary driver. SCC does not detect label-swap pathologies: an embedding that maps cell
types to the wrong regions still produces confident predictions. CKA and
RCS share this limitation---geometric relationships are preserved under
label permutation---so this is not a comparative weakness of SCC. CCAL requires cell-type labels in both source and target
domains. SCC requires labels only in the source domain: the target
data can be fully unlabeled, making SCC directly applicable as a
transfer-quality estimate before any target annotation is available.
MMD is also label-free. The within-tissue Kendall $\tau$ test has only $n = 3$ models per
tissue, making individual tissue-level $\tau$ values coarse ($\tau \in
\{-1, -1/3, +1/3, +1\}$); the consistency of positive $\tau$ across
all 23 independent tissues provides stronger evidence than the pooled
statistic alone, because it cannot be driven by a few outlier tissues. SCC's design was motivated by the theoretical observation that
relational metrics are invariant to cluster separability; the
evaluation framework (BH correction, primary success criterion) was
frozen before computation. The noise dose-response experiment uses Gaussian additive
...
The spectral-gap ratio (SGR) was tested as an alternative to CKA that
exploits eigenvalue structure. Four hypotheses were tested: HS1
...
\paragraph{Results.} HS1: baselines have higher $z$-scores (mean 141
vs.\ 88; $p > 0.99$). HS2: partial $\rho = -0.11$ ($p = 0.99$). HS3:
partial $\rho = 0.001$ ($p = 0.99$). All four hypotheses fail.

\paragraph{Interpretation.} Spectral concentration does not solve the
discriminative-validity problem. The failure is informative: it
eliminates eigenvalue normalization as a fix and establishes that the
deficiency is structural to small-$k$ centroid inputs. Any projection
of biologically structured data produces centroid matrices with
concentrated spectra, regardless of whether the projection learned
