...
68\%). We tested four transfer-aware alternatives under a registered
...
$\rho = 0.73$ with transfer F1 ($p < 10^{-4}$, BH-corrected), 86\%
pairwise model discrimination, and concordant Kendall $\tau$ in 21 of
21 tissues. MMD and class-conditional alignment (here CCAL; distinct from canonical
correlation analysis) also pass BH-correction. The failure of existing metrics and success of SCC
share a common explanation: relational metrics measure geometric
similarity but are invariant to cluster separability, while SCC
...
data---achieves $\rho = 0.73$ with transfer F1 ($p < 10^{-4}$,
BH-corrected across nine metrics), 86\% pairwise model discrimination,
and concordant Kendall $\tau$ in 21 of 21 tissues. Fifth, two attempted
...
strongest single statistic ($\rho = 0.73$, unanimous tissue-level
concordance).
...
separability and conditional alignment. We specified four candidates in
...
multiple-comparison correction were frozen before computation).

Source classifier confidence (SCC) trains a cell-type classifier
(logistic regression, $\text{max\_iter} = 1000$) on source embeddings,
applies it to target cells, and reports the mean maximum predicted
probability. Maximum mean discrepancy (MMD) measures kernel-based
...
\begin{table}[t]
\centering
\caption{Transfer-aware metric comparison across 21 tissues
$\times$ 3 models (63 conditions). Spearman $\rho$ with transfer F1
...
$p$-values BH-corrected across nine metrics.}
\label{tab:scc}
\small
...
\midrule
SCC              & $+0.733$ & $[+.59,\;+.82]$ & $<0.001$ & 85.7\% & 21/21 & $<0.001$ \\
MMD              & $+0.704$ & $[+.56,\;+.80]$ & $<0.001$ & 84.1\% & 20/21 & $<0.001$ \\
CCAL             & $+0.575$ & $[+.39,\;+.72]$ & $<0.001$ & 82.5\% & 20/21 & $<0.001$ \\
\midrule
RCS (baseline)   & $+0.252$ & $[+.02,\;+.45]$ & $0.060$  & 68.3\% & 16/21 & $0.040$ \\
RCS (PCA-10)     & $+0.214$ & $[-.06,\;+.45]$ & $0.092$  & 61.9\% & 15/21 & $0.088$ \\
RCS (trimmed)    & $+0.258$ & $[+.03,\;+.46]$ & $0.060$  & 68.3\% & 17/21 & $0.016$ \\
RCS (normalized) & $+0.219$ & $[-.04,\;+.45]$ & $0.092$  & 69.8\% & 15/21 & $0.088$ \\
RCS (combined)   & $+0.297$ & $[+.03,\;+.52]$ & $0.032$  & 65.1\% & 16/21 & $0.040$ \\
\midrule
PAD              & $+0.306$ & $[+.07,\;+.52]$ & $0.032$  & 62.5\% & 12/21 & $0.664$ \\
\bottomrule
\end{tabular}
\end{table}

SCC achieves the strongest F1 prediction on every test
(Table~\ref{tab:scc}). The pooled Spearman correlation is $\rho = +0.73$
($p < 10^{-4}$, BH-corrected), pairwise model discrimination is 85.7\%
(binomial $p < 10^{-4}$), and the within-tissue Kendall $\tau$ is
positive in all 21 tissues (sign-test $p < 10^{-4}$). MMD and CCAL
produce concordant results ($\rho = +0.70$ and $+0.58$), providing
convergent evidence from three distinct theoretical angles.

PAD---the metric most directly grounded in domain-adaptation
theory---fails pairwise discrimination (62.5\%, non-significant after BH correction). PAD
...
quantifies. The correlation is $\rho = 0.73$ ($p < 10^{-4}$), and the
concordance is unanimous (21/21 tissues). MMD and CCAL converge on the
same conclusion from distributional-divergence and centroid-alignment
perspectives. The pattern is clear: metrics that measure properties
relevant to classification (separability, conditional alignment,
decision-boundary transfer) predict transfer quality; metrics that
measure geometric similarity alone (RCS, CKA, PAD) do not.
...
hyperparameters beyond the classifier itself, achieves $\rho = 0.73$
with transfer F1, and is computable from the same data any
cross-technology evaluation already uses. Fifth, treat RCS and scGraph
...
regime. SCC uses the same logistic-regression classifier as the F1
ground truth, so the strong correlation partly reflects shared
classifier capacity rather than an independent signal; other classifiers
(random forest, kNN) would test generality. SCC does not detect
...
($n = 63$) is the more informative statistic. The transfer-aware metrics
...
Four transfer-aware metrics were computed per (tissue, model) condition
across 21 tissues and three models (Geneformer, scGPT, scVI), yielding
63 conditions. Ground truth is macro-averaged logistic-regression F1 on
shared cell types (same classifier as the main panel). All metrics use
the same Census download, random seed (20260727), and cell subsample
($n = 2{,}000$ per side, $\geq 8$ shared cell types).

\paragraph{Source classifier confidence (SCC).} Train a logistic
regression classifier ($\text{max\_iter} = 1000$) on source embeddings
restricted to shared cell types. Apply to target cells. The metric is
the mean maximum predicted probability across target cells.
...
correction is applied across all nine metrics (five RCS variants + four
new metrics) separately for each of three test types: pooled Spearman
correlation, pairwise binomial test, and within-tissue Kendall $\tau$
sign test. The primary success criterion is BH-corrected $p < 0.05$ on
the within-tissue Kendall $\tau$ sign test.
...
trimmed-mean centroids, column-max normalization, combined) and four new
transfer-aware metrics (SCC, MMD, CCAL, PAD). Definitions in
Section~\ref{sec:sccmethod}.

\paragraph{Evaluation framework.} Three tests per metric: (1) pooled
Spearman $\rho$ with transfer F1 ($n = 63$); (2) pairwise model
discrimination accuracy with binomial test; (3) within-tissue Kendall
$\tau$ sign test ($n = 21$ independent observations). BH correction
across all nine metrics, applied separately to each test type. Primary
success criterion: BH-corrected $p < 0.05$ on the within-tissue Kendall
$\tau$ sign test.

\paragraph{Results.} Three metrics pass the primary criterion
decisively: SCC (21/21 positive $\tau$, BH $p < 0.001$), MMD (20/21,
BH $p < 0.001$), and CCAL (20/21, BH $p < 0.001$). Three RCS
variants pass marginally: RCS-trimmed (17/21, BH $p = 0.016$),
RCS-baseline (16/21, BH $p = 0.040$), and RCS-combined (16/21,
BH $p = 0.040$). RCS-PCA10 (15/21, BH $p = 0.088$),
RCS-normalized (15/21, BH $p = 0.088$), and PAD (12/21,
BH $p = 0.664$) fail. Full results in Table~\ref{tab:scc}.
