...
\begin{table}[t]
\centering
\caption{Transfer-aware metric comparison. Spearman $\rho$ with
transfer F1, pairwise model discrimination accuracy (binomial test),
and within-tissue Kendall $\tau$ sign test. Pairwise and $\tau$
columns are from the original 3-model panel (23 tissues $\times$ 3
models, 69 conditions); Spearman $\rho$ replicates on the expanded
4-model panel (92 conditions, adding UCE;
Section~\ref{sec:sccmethod}). All $p$-values BH-corrected across
twelve metrics. SCC variants use four classifier families; F1 ground
...
in gene space (pre-registered analysis plan, SHA-256:
\texttt{2c0321f0\ldots bb93}).
...
$p < 10^{-7}$), satisfying the pre-registered precondition that MGR
falls within the convergent-validity window ($\rho \in [0.25, 0.80]$):
high enough to measure a related property, low enough to constitute
...
within-tissue Kendall $\tau$ is positive in 23 of 23 tissues.
...
regime. SCC's correlation with F1 survives replacement of the
SCC classifier with kNN ($\rho = 0.56$), random forest ($\rho = 0.57$),
or linear SVM ($\rho = 0.55$), ruling out shared logistic-regression
...
MMD is also label-free. The within-tissue Kendall $\tau$ test has only $n = 4$ models per
tissue, making individual tissue-level $\tau$ values coarse; the
consistency of positive $\tau$ across 23 independent tissues provides
stronger evidence than the pooled statistic alone, because it cannot
be driven by a few outlier tissues. SCC's correlation with MGR is
...
resolved by these data. SCC's design was motivated by the theoretical observation that
