# Preflight Bio — Experiment Preregistration (v2)
...
- Degradation = F1_source − F1_target
- Relative degradation = (F1_source − F1_target) / F1_source

### Hypotheses

**H0.1 (PRIMARY):** Preflight composite score and degradation are negatively correlated across all ~25 pairs (Spearman ρ < −0.5, p < 0.01).  
*Rationale:* Higher composite score means better predicted transportability, which should correspond to less degradation. The ρ < −0.5 threshold matches the partial r = 0.61 previously published for M1 alone on cohort data — the composite should match or exceed a single module.

**H0.2:** Pairs with composite Tier ≥ 5 have mean degradation < 0.10 (F1 drop < 10 points).  
*Rationale:* "Acceptable" tier should correspond to acceptable real-world performance.

**H0.3:** Pairs with composite Tier ≤ 2 have mean degradation > 0.30 (F1 drop > 30 points).  
*Rationale:* "Very Poor" / "Failure" tier should correspond to severe performance collapse.

**H0.4:** M2 domain shift AUC alone achieves ρ < −0.4 with degradation.  
*Rationale:* M2 is expected to be the strongest single predictor based on prior MSMS results. This establishes the single-module baseline that the composite must beat.

**H0.5:** The composite outperforms the best single module (composite |ρ| > best single-module |ρ|).  
*Rationale:* If the composite doesn't beat the best individual module, the weighting scheme adds no value and should be replaced.

### Decision criteria

- **H0.1 passes:** The composite is a validated transfer-failure predictor. This is the fundable, publishable, product-ready claim.
- **H0.1 fails (ρ > −0.5):** The composite weights need recalibration. Use the ~25 pairs as a training set to learn weights via cross-validated regression, then validate on held-out pairs.
...
2. For each pair:
   ...
   e. Compute degradation = F1_source − F1_target
3. Compute Spearman ρ(composite_score, degradation) across all pairs
4. Compute Spearman ρ(module_i_score, degradation) for each module individually
5. Test H0.2 and H0.3 by grouping pairs by tier
6. Plot: composite score vs degradation, with pair-type as color
7. Save all results to results/composite_validation/

### Contingencies

- Some tissue × assay combinations may have too few cells or too few cell types for a meaningful probe. Pairs with < 5 shared cell types (present in both source and target with ≥ 10 cells each) are dropped. Record the number of dropped pairs.
...
**H4.3:** M7 ecological bias score ≥ 0.5 (low bias). (Weaker hypothesis — disease prevalence genuinely varies by dataset.)
...
**Experiment 0 is the keystone.** If H0.1 passes (ρ < −0.5), the composite is a validated transfer predictor. If it fails, the weights need recalibration using the same pairs as training data. Either outcome is informative and publishable.
