...
\title{Cell-Type Centroid Alignment Predicts Cross-Assay Transfer\\Where scIB Bio-Conservation Metrics Fail}
...
performance gaps. Among competitive embeddings, overall inversion rates
reached 46--58\%, driven by near-tie comparisons where
64--86\% of rankings inverted. These rankings were highly stable to Leiden resolution and
...
the boundary of this failure: scIB bio-conservation scores positively
predicted transfer ($\rho = 0.63$, permutation $p = 0.0014$) when
protocol-level artifacts are removed. Cell-type centroid alignment
recovers predictive power under cross-assay shift. CKA and Procrustes
similarity on cell-type centroids predict cross-assay transfer in a
25-tissue evaluation (partial $\rho = +0.54$, $+0.66$;
$p < 10^{-4}$), and CKA generalizes to two of three independent
datasets (pancreas $\rho = +0.59$, Tabula Sapiens $\rho = +0.60$).
The third dataset (PBMC) shows a ceiling effect where all technologies
produce near-identical CKA on canonical cell types. These results
show that conditioning on biological identity---cell-type centroid
preservation---restores predictive validity under cross-assay shift,
and motivate a minimum standard
for embedding benchmarks: matched-dimensionality baselines, controlled
degradation tests, and downstream validation under the intended
deployment shift.
\end{abstract}

\noindent\textbf{Keywords:} single-cell foundation models, scIB, construct validity, embedding evaluation, cross-assay transfer, centered kernel alignment


\section{Introduction}
\label{sec:intro}

Single-cell foundation model benchmarks consistently find that PCA on raw counts matches or exceeds large pretrained models across most evaluation axes \citep{wu2025scfm, vcbench2026, unifiedscfm2026, cellxgeneintegration2026}. The pattern is robust across benchmark designs. The question is whether this reflects a genuine ceiling on learned representations, or insensitivity in the evaluation metrics.
...
Cell-type centroid metrics recover predictive validity under cross-assay shift. CKA and Procrustes on cell-type means predict cross-assay transfer (partial $\rho = +0.54$, $+0.66$; $p < 10^{-4}$) on a 25-tissue Census panel, and CKA generalizes to two of three independent datasets.
...
The failure is specific to the cross-assay setting. Matched-assay transfer removes the protocol-artifact axis (capture efficiency, UMI distributions, gene detection sensitivity), and the metrics recover predictive validity. scIB bio rankings are valid across tissues at matched assay and require downstream-probe validation for cross-assay comparisons.
...
The inversions are intrinsic: the metrics produce the same rankings regardless of resolution or neighborhood size.
...
\subsection{Cell-type structural metrics predict cross-assay transfer}
\label{sec:structure}

Cell-type centroid alignment measures whether the biological coordinate system---the relative positions of cell-type means---transfers across assays. We pre-registered two centroid-based metrics and evaluated them on an expanded Census panel: 25 auto-discovered tissues ($\geq 200$ cells per assay, $\geq 8$ shared cell types), eight embedding methods (six contenders including two Geneformer-v2 variants, two null baselines), 104 contender conditions. Cell-type CKA \citep{kornblith2019similarity} computes centered kernel alignment on centroid matrices. Procrustes similarity returns $1 - d_{\mathrm{Proc}}^2$ after optimal orthogonal alignment of centroids.

Both metrics positively predict cross-assay transfer F1 (partial Spearman $\rho$ controlling for embedding dimensionality, tissue-stratified permutation $p$-values). Procrustes similarity achieves partial $\rho = +0.657$ ($p < 10^{-4}$, 95\% CI $[0.54, 0.72]$); cell-type CKA achieves $\rho = +0.536$ ($p < 10^{-4}$, CI $[0.44, 0.66]$). These correlations reflect cell-type centroid preservation, a property invisible to distributional metrics. Two embeddings can differ substantially in global geometry (large subspace divergence, high domain-classifier AUC) and still arrange cell-type means identically. Raw subspace geometry anti-predicts transfer ($\rho = +0.38$, wrong sign; Supplementary~\ref{app:modules}). Cell-type CKA predicts correctly ($\rho = +0.54$) on the same model panel, confirming that the evaluation gap depends on what the metric conditions on.


\subsection{External validation on independent datasets}
\label{sec:external}

Cell-type CKA was pre-registered and tested on three datasets with no overlap with the Census evaluation: pancreas \citep{luecken2022scib} (6 scRNA-seq technologies, 56 assay pairs), Tabula Sapiens \citep{tabulasapiens2022} (10x and Smart-seq2 across 8 organs, 16 pairs), and PBMC \citep{ding2020systematic} (7 technologies, 28 pairs). Embedding models are PCA baselines only (all-gene and HVG at $d = 50$ and $d = 200$, random projection, untrained encoder); no foundation models are included, isolating metric behavior from model choice. Transfer F1 (logistic regression, macro-averaged) is the ground truth.

CKA generalizes to two of three datasets (Table~\ref{tab:external}). In pancreas, CKA achieves partial $\rho = +0.592$ ($p = 0.0001$); in Tabula Sapiens, $\rho = +0.604$ ($p = 0.035$). Procrustes similarity is consistently positive across all three datasets ($\rho = +0.56$, $+0.73$, $+0.11$), matching the Census direction ($\rho = +0.66$). It reaches significance in none, likely a power limitation from fewer technology pairs per dataset. Field baselines (MMD, domain classifier AUC) fail in all three datasets. Target-domain silhouette is significant in all three ($\rho = +0.17$ to $+0.66$), likely confounded: target assays with well-separated cell types are easier to classify regardless of source embedding quality. PBMC shows a ceiling effect: all seven technologies produce CKA $> 0.92$ on canonical cell types (T cells, B cells, monocytes), leaving insufficient variance for discriminative ranking ($\rho = +0.089$, $p = 0.874$; Supplementary~\ref{app:pbmc}). CKA predicts transfer when cell-type difficulty varies across technologies and fails only when the task is uniformly easy.

\begin{table}[t]
\centering
\caption{External validation of cell-type structural metrics on three independent datasets (no Census data, no foundation models). Partial Spearman $\rho$ controlling for embedding dimensionality; tissue-stratified permutation $p$-values. Bold: significant at $\alpha = 0.05$. PBMC CKA is null due to variance compression (all technologies $> 0.92$).}
\label{tab:external}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
...
\paragraph{Handling scVI's fixed dimension.} scVI produces $d = 50$ embeddings by default. We include scVI in analyses without dimensionality controls and exclude it from matched-dimensionality comparisons.

\paragraph{Expanded Census panel.} Cell-type structural metrics (Section~\ref{sec:structure}) were evaluated on 25 auto-discovered tissues from CellxGene Census ($\geq 200$ cells per assay, $\geq 8$ shared cell types). The model panel adds two Geneformer-v2 variants (104M and 316M parameters) to the five methods above, yielding eight models (six contenders, two null baselines), 154 total conditions, 104 contenders.

\paragraph{External validation datasets.} Three independent datasets were used for external validation (Section~\ref{sec:external}), each with PCA-only embeddings (all-gene and HVG at $d = 50$ and $d = 200$, random projection, untrained encoder). Pancreas: the scIB benchmark dataset \citep{luecken2022scib}, six technologies (CelSeq, CelSeq2, Fluidigm C1, inDrop, Smart-seq2, SMARTer), 56 assay pairs, 224 contender conditions. Tabula Sapiens \citep{tabulasapiens2022}: 10x and Smart-seq2 across eight organs, 16 pairs, 64 contenders. PBMC \citep{ding2020systematic}: seven technologies (10x v2, 10x v3, Drop-seq, inDrops, Seq-Well, CEL-Seq2, Smart-seq2), 28 pairs, 112 contenders; cell-type annotations from SCP424 mapped via cell barcode matching (85\% match rate, 26{,}009 cells).
...
\subsection{Cell-type structural metrics}

Cell-type CKA \citep{kornblith2019similarity} computes centered kernel alignment on the $k \times d$ centroid matrices (mean embedding per shared cell type) between source and target assays, using linear kernels. Procrustes similarity applies orthogonal Procrustes analysis to column-centered centroid matrices and returns $1 - d_{\mathrm{Proc}}^2$ after optimal rotation. Both metrics require cell-type labels and are evaluated only on the $k$ cell types shared between source and target.
...
The scIB bio-conservation suite reliably identifies the best model (80--100\% accuracy on clear-gap comparisons) and loses discriminative power among competitive embeddings (64--86\% inversion rate on near-ties). A metric can be discriminative (trained $>$ null), stable (Kendall's $W > 0.80$), and still wrong about which embedding to deploy. Discriminative validity and predictive validity are orthogonal properties.
...
The matched-assay replication is the strongest finding. scIB metrics positively predict cross-tissue transfer ($\rho = +0.63$, $p = 0.0014$) while inverting cross-assay rankings among competitive embeddings. This dissociation establishes a boundary condition: the failure appears when comparing across assay technologies and disappears at matched assay. The result is consistent with protocol-level artifacts (capture efficiency, UMI distributions, gene detection sensitivity) driving the inversions, though the present design does not isolate individual artifact contributions. scIB bio rankings are valid within an assay platform and require a downstream probe for cross-assay comparisons.

Cell-type centroid alignment addresses this failure mode. CKA and Procrustes on cell-type centroids positively predict cross-assay transfer in the Census evaluation (partial $\rho = +0.54$, $+0.66$; Section~\ref{sec:structure}), and CKA generalizes to two of three independent datasets (pancreas $\rho = +0.59$, Tabula Sapiens $\rho = +0.60$; Section~\ref{sec:external}). The third dataset (PBMC) shows a ceiling: all technologies produce near-identical CKA on canonical cell types (Supplementary~\ref{app:pbmc}). A practical evaluation protocol would pair a label-free geometric filter with a centroid-based metric requiring cell-type annotations, followed by a downstream probe under the intended deployment shift.
...
\paragraph{Limitations.} Primary experiments use CellxGene Census (v2023-12-15) across 4--25 tissues; external validation (Section~\ref{sec:external}) extends to three independent datasets. All evaluations use cross-assay cell-type transfer F1 as ground truth. Additional ground truths (perturbation prediction) would further constrain the scope of the failure. The scIB audit spans five embedding methods across three architectural families (transformer, VAE, PCA); the expanded Census panel adds two Geneformer-v2 variants. Expansion to additional models and assay pairs would increase statistical power. The inversion analysis rests on 24 contender comparisons; the clear-winner cell ($n = 10$) is dominated by Geneformer-versus-field contrasts. Brain tissue has no clear F1 winners (all pairwise $|\Delta\text{F1}| < 0.10$), making its near-tie inversions expected. We release the protocol alongside our failing modules: the checklist is the contribution; the module failures show that the checks are non-trivial.
...
The PBMC dataset \citep{ding2020systematic} provides seven scRNA-seq technologies (10x Chromium v2 and v3, Drop-seq, inDrops, Seq-Well, CEL-Seq2, Smart-seq2) applied to the same donor PBMCs, with author-provided cell-type annotations from the Single Cell Portal (SCP424). Annotations were mapped to GEO barcodes (GSE132044) via cell barcode matching with method-specific normalization (85\% match rate, 26{,}009 cells, 9 cell types after excluding unassigned).

Cell-type CKA ranges from 0.92 to 0.97 across all 28 source-target technology pairs. This variance compression explains the null correlation with transfer F1 ($\rho = +0.089$, $p = 0.874$): the metric cannot rank technologies on a property where they score nearly identically. The same result obtains with marker-based cell-type annotation ($\rho = +0.163$, $p = 0.849$), confirming that annotation quality is not the limiting factor.

The ceiling reflects the dataset's cell-type composition. T cells, B cells, NK cells, and monocytes are well-separated in any reasonable embedding; no technology fails to capture this structure. In contrast, the pancreas dataset contains rare endocrine subtypes (alpha, beta, delta, PP cells) that are harder to resolve, providing sufficient CKA variance for the correlation to emerge. A metric that succeeds when task difficulty varies and fails when difficulty is uniformly low behaves as expected for a measure of cell-type structural preservation.
