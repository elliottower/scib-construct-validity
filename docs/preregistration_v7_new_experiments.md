...
**Status**: DRAFT — awaiting external review before SHA freeze and execution.
...
1. **Exp 4**: Comparison to existing transferability metrics (RankMe, MMD, C2ST) — reframed as "they falsely certify where we abstain"
2. **Exp 4b**: Degenerate embedding safety check — verify M4 catches BoG-512 degeneracy

**Priority 2 — Broaden the panel (additive):**
3. **Exp 5**: Cross-tissue shift (same assay, different tissue)
4. **Exp 6**: Cross-disease shift (same tissue + assay, healthy vs. diseased)
5. **Exp 7**: scGPT embeddings (third foundation model)

All experiments use the frozen v6 scorer. No modifications to scoring logic, composite weights, or tier thresholds. The only new code is experiment harness scripts that call the existing scorer.
...
- **H4.1**: Preflight composite achieves higher |Spearman rho| with relative degradation than RankMe, MMD, or C2ST individually.
- **H4.2**: Preflight composite achieves higher AUROC for shifted-vs-control classification than any single competing metric.
- **H4.3**: C2ST performance is similar to Preflight's M2 module alone (since they measure the same thing), but the full composite outperforms C2ST by incorporating subspace geometry (M1) and dimensionality (M4).
- **H4.4**: On pairs where the BoG-PCA embedding scores Tier ≥ 5 (degenerate M4), competing metrics (RankMe, MMD) also produce passing scores — i.e., they falsely certify a degenerate embedding as transferable. Preflight's M4 module flags the degeneracy that single-metric approaches miss.
- **H4.5**: The Preflight composite has a lower false-certification rate than any competing metric, where "false certification" is defined as scoring a pair above the median threshold when relative degradation exceeds 0.30.
...
## Experiment 4b: Degenerate Embedding Safety Check

### Motivation

The Exp 1 and Exp 2 results show that bag-of-genes PCA-512 achieves Tier 5–6 despite having the worst probe F1. This is the most dangerous failure mode for a naive user: a degenerate embedding receives a passing grade. The two-gate structure should catch this via M4 (participation ratio = 0 for BoG-512 in Exp 1), but we must verify that the overall system flags the degeneracy rather than letting the high M1/M2 scores mask it.

### Design

Using the already-computed Exp 1 and Exp 2 results:
1. Check whether BoG-512's M4 = 0 triggers a warning in the diagnostic output.
2. Verify that if a "worst-tier gate" or "minimum module threshold" were applied (i.e., any module below Tier 2 caps overall tier at 3), BoG-512 would be flagged.
3. Propose and test a concrete rule: **if M4 < 0.05, append a "DEGENERATE DIMENSIONALITY" warning regardless of overall tier**.

### Hypotheses

- **H4b.1**: BoG-512 has M4 participation ratio < 0.01 in all tissues tested (Exp 1 + Exp 2 data).
- **H4b.2**: Under a worst-module gate (any module ≤ Tier 1 caps overall to ≤ Tier 3), BoG-512 would be reclassified from Tier 5–6 to Tier ≤ 3 in all conditions.
- **H4b.3**: No non-degenerate embedding (Geneformer, scVI) has M4 < 0.05 in any condition tested.
...
## Experiment 6: Cross-Disease Shift
...
- **H6.2**: Cancer pairs (lung adenocarcinoma, HCC) show larger shifts than non-cancer disease pairs (IPF, AD), because cancer involves more dramatic cell-type composition changes.
- **H6.3**: M2 domain AUC is lower for cross-disease than for cross-assay shifts (disease shifts are subtler than technical batch effects).
- **H6.4**: M3 direction stability is lower for disease pairs than negative controls (the discriminant directions change under disease).
...
- Exploratory analyses (correlations, module breakdowns beyond hypotheses) are labeled as exploratory.
