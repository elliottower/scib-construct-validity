# Preflight Bio — Experiment Preregistration (v3)

**Status:** DRAFT — pending review before SHA freeze  
**Date:** 2026-07-10  
**Author:** Elliot Tower  
**Reviewer:** (Perplexity / external)  
**Changes from v2:** 5 fixes from Perplexity review — (1) relative degradation as primary endpoint, (2) shifted-pairs-only as primary analysis, (3) partial-ρ for H0.1 with corrected threshold, (4) within-group ρ for cross-tissue cluster, (5) H0.5 tests composite vs M2 specifically.
...
- Per-module scores (M1, M2, M3, M4, M6, M7 where active)
...
*Rationale:* Sanity check. Including negative controls should strengthen the correlation. If H0.1 passes but H0.1b fails, something is wrong with the negative control behavior.
...
6. Compute partial ρ per module (M1, M2, M3, M4, M6, M7) on shifted pairs — report all, test H0.5 against M2 only
