...
transfer, achieving Spearman $\rho = 0.67$ ($p < 10^{-11}$,
...
data---achieves $\rho = 0.67$ with transfer F1 ($p < 10^{-11}$,
...
confounded by dimensionality; Additional file~1).
...
with F1 ($\rho = +0.67$, $p < 10^{-11}$, BH-corrected across twelve
...
ranking ($\rho = +0.09$, $p = 0.87$; Additional file~1). CKA
...
models and 92 conditions is $\rho = 0.67$ ($p < 10^{-11}$, BH-corrected), and the
...
relationship ($\rho \geq 0.54$), even though these classifiers produce
...
(Additional file~1). Null saturation is structural to
...
(Additional file~1). Independently,
...
\paragraph{Additional file 1} --- Supplementary material.
Four supplementary sections: PBMC ceiling effect, geometric modules,
spectral-gap experiment, and noise dose-response calibration.
Includes Supplementary Tables~S1--S3 and Supplementary Figures~S1--S3.
