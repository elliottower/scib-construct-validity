...
\title{scIB Bio-Conservation Metrics Invert Cross-Assay\\Model Rankings at Chance}
...
\begin{abstract}
Single-cell foundation model benchmarks rely on scIB bio-conservation metrics (NMI, ARI, silhouette, cLISI, isolated-label ASW) to rank embedding quality, yet these metrics invert 46--58\% of pairwise model rankings against ground-truth cross-assay transfer F1---at or worse than chance. An expanded panel adds Geneformer V2 (104M and 316M parameters) to the contender set, increasing architectural diversity while a cross-tissue replication localizes the failure. The same metrics positively predict cross-tissue transfer ($\rho = +0.63$, $p = 0.0014$), recovering predictive validity only when protocol-level artifacts (capture efficiency, UMI distributions, gene detection sensitivity) are removed. Two confirmatory tests show that metric \emph{rankings} are near-perfectly stable across hyperparameters ($\bar{\tau} \geq 0.80$) while metric \emph{values} are non-monotonic under controlled noise in 8 of 10 metrics---the inversions are intrinsic, not artifacts of analysis choices. We apply the same three-check construct-validity protocol to five of our own geometric modules before turning it on scIB; the protocol eliminates four of five, establishing that the checks are non-trivial. The underlying mechanism is the Johnson--Lindenstrauss lemma: at $d \geq O(\log n / \varepsilon^2)$, random projections preserve the distances and neighbor graphs on which most geometric metrics depend, so any metric that is a function of these quantities will score random projections comparably to trained models. No scIB bio-conservation metric reliably predicts cross-assay transfer, but the metrics work for cross-tissue comparisons within an assay platform.
\end{abstract}

\noindent\textbf{Keywords:} single-cell foundation models, scIB, construct validity, embedding evaluation, Johnson--Lindenstrauss, cross-assay transfer
...
We apply a three-check construct-validity protocol---null-model discrimination, cross-dimensionality robustness, sign-correctness---to the scIB suite. The scIB bio-conservation metrics pass the easy check (trained $>$ random) and fail the hard one (46--58\% pairwise inversions against ground-truth transfer F1). A pre-registered cross-tissue replication localizes the failure: scIB metrics positively predict cross-tissue transfer ($\rho = +0.63$) while inverting cross-assay rankings at chance. The underlying mechanism is the Johnson--Lindenstrauss lemma: at $d \geq O(\log n / \varepsilon^2)$, random projections preserve the distances and neighbor graphs on which most geometric metrics depend \citep{johnsonlindenstrauss1984}. The same confound appears in neural population geometry, where 19 geometric metrics are confounded by neuron count via the same mechanism \citep{tower2026bracket}. We apply the protocol to five geometric modules of our own before turning it on scIB; it eliminates four of five (Supplementary~\ref{app:modules}), establishing that the checks are non-trivial.
...
Ten scIB metrics were evaluated on eight embeddings---the original six (Geneformer, scVI, scGPT, bag-of-genes PCA-512, random projection, untrained encoder) plus Geneformer V2-104M ($d = 768$) and V2-316M ($d = 1{,}152$)---across four tissues ($n = 2{,}000$ per side). Ground-truth transfer performance is macro-averaged cell-type kNN F1 from source to target. Our claim is specific: scIB metrics lack predictive validity for cross-assay transferability, the question that current foundation model benchmarks use them to answer. A metric valid for integration evaluation may still be invalid when repurposed for model ranking.
...
\paragraph{Pairwise ranking accuracy.} The harder question is \emph{which} trained model to deploy. We defined a contenders set---Geneformer (V1), Geneformer V2-104M, Geneformer V2-316M, scVI, scGPT, and bag-of-genes PCA-512---and counted pairwise rank inversions against ground-truth F1.
...
\begin{table}[t]
\centering
\caption{Pairwise inversion rate against ground-truth F1 on the original contenders-4 set (Geneformer V1, scVI, scGPT, BoG-PCA; 6 pairs $\times$ 4 tissues $= 24$ comparisons), split by F1 gap. Clear winners ($|\Delta\text{F1}| > 0.10$): the metric's ranking call matters. Near-ties ($|\Delta\text{F1}| \leq 0.10$): the correct ranking is ambiguous. Tissue-clustered bootstrap 95\% CI on the overall rate. Results from the expanded contenders-6 set (adding V2-104M and V2-316M; 15 pairs $\times$ 4 tissues $= 60$ comparisons) are reported in Supplementary~\ref{app:expanded}.}
...
The mechanism is straightforward: scIB bio metrics reward cluster compactness of known cell types, a property that trained-versus-random separates easily. Ranking two competent embeddings by transfer quality requires sensitivity to cross-assay alignment, a property the suite was never designed to measure.
...
We apply the same three-check protocol to five geometric modules of our own design before turning it on scIB; the protocol eliminates four of five (Supplementary~\ref{app:modules}). This self-audit establishes that the checks are non-trivial and catch real failures, including in metrics designed by the protocol's authors.
...
Scorer source code, hyperparameters, dataset specifications, and hypothesis thresholds were frozen via SHA-256 hashing before data access. Three implementation bugs were identified during development and fixed before any confirmatory experiment; the corrected scorer was refrozen (v7). SHA-256 hashes and the full deviation log are in the online repository.
...
The scIB bio-conservation suite reliably identifies the best model (80--100\% accuracy on clear-gap comparisons) but ranks the remaining field at chance (64--86\% inversion rate on near-ties). Adding Geneformer V2 at two scales (104M and 316M parameters) to the contender set increases both the number of pairwise comparisons and the range of embedding dimensions (50--1{,}152), providing a stronger test of whether scale resolves the inversions. A metric can be discriminative (trained $>$ null), stable (Kendall's $W > 0.80$), and still wrong about which embedding to deploy. Discriminative validity and predictive validity are orthogonal properties.

The JL confound provides a principled explanation for why baselines keep winning across the field's benchmarks \citep{johnsonlindenstrauss1984}. At $d \geq O(\log n / \varepsilon^2)$, random linear maps preserve the data's distance structure, covariance eigenspaces, and neighborhood graphs to $(1 \pm \varepsilon)$. Any metric that is a function of these quantities will score random projections comparably to trained models. The same confound appears in neural population geometry (19 metrics confounded by neuron count; \citealp{tower2026bracket}) and RNA foundation model evaluation (Grassmannian distance reproduced by 3-mer frequencies). The recurrence across biological domains suggests geometric evaluation in high dimensions is vulnerable whenever $d \gg \log n$.

The cross-tissue replication is the strongest finding. scIB metrics positively predict cross-tissue transfer ($\rho = +0.63$, $p = 0.0014$) while inverting cross-assay rankings at chance. This dissociation localizes the failure to protocol-level artifacts (capture efficiency, UMI distributions, gene detection sensitivity) that cross-tissue comparison at matched assay removes. The implication is specific: scIB bio rankings can be trusted within an assay platform but require a downstream probe across assays.

Geometric metrics retain a role as label-free diagnostics during model development, provided their rankings are confirmed by a small downstream probe. M3 (direction stability) captures a property---eigenvector stability across resamples---that random projections do not reproduce, and could serve as an early-stage filter. Whether M3 predicts downstream transfer remains undetermined ($\rho = +0.30$, CI $[-0.34, +0.80]$, $n = 12$); the expanded panel ($n = 20$ with V2 variants) narrows the CI but a definitive answer requires $n \approx 85$ conditions. For future benchmark papers, the minimum standard should be a matched-dimensionality baseline plus a downstream probe to anchor any geometric ranking. Report NMI or ARI (not both), silhouette-label, and graph connectivity separately; do not collapse them into a single overall score.

\paragraph{Limitations.} All primary experiments use CellxGene Census (v2023-12-15), four tissues, and cross-assay cell-type transfer F1 as ground truth. Additional ground truths (perturbation prediction) would further constrain the scope of the cross-assay failure. The current panel spans seven embedding methods across three architectural families (transformer at four scales, VAE, PCA); scPRINT and Nicheformer were pre-registered but excluded due to gene-identifier incompatibility with the CellxGene Census API, a limitation of standardized evaluation rather than of the models themselves. The inversion analysis rests on contender comparisons across four tissues; the clear-winner cell is dominated by Geneformer-versus-field contrasts. Brain tissue has no clear F1 winners (all pairwise $|\Delta\text{F1}| < 0.10$), making its near-tie inversions expected. We release the protocol alongside our failing modules: the checklist is the contribution; the module failures show that the checks are non-trivial.
...
\section{Expanded panel: Geneformer V2}
\label{app:expanded}

The pre-registered expansion (PREREGISTRATION\_SCIB\_EXPANDED\_PANEL.md, SHA-256 frozen before data access) added four models to the contender set: Geneformer V2-104M ($d = 768$), Geneformer V2-316M ($d = 1{,}152$), scPRINT, and Nicheformer. Embeddings were generated on the same four tissues using identical Census queries (v2023-12-15, seed 20260713, $n = 2{,}000$ per side).

\paragraph{Successful models.} Geneformer V2-104M and V2-316M produced embeddings for all 8 conditions (4 tissues $\times$ 2 roles). V2 embeddings were generated using the Morpheus/Cole tokenization protocol: raw counts $\to$ CP10K normalization $\to$ divide by per-gene medians from the V2 training corpus $\to$ rank-sort descending $\to$ top-2048 tokens $\to$ BertForMaskedLM $\to$ mean-pool last hidden state.

\paragraph{Excluded models.} scPRINT and Nicheformer were excluded because their inference pipelines could not resolve CellxGene Census gene identifiers. scPRINT's scdataloader preprocessor requires bionty-registered gene records; even after populating the ontology, the Census gene identifiers (Ensembl format) failed to map, with all 60{,}664 genes removed. Nicheformer's tokenizer performs an outer join between the input AnnData and a 20{,}310-gene reference vocabulary; Census's broader gene set produces token indices that overflow the model's embedding table (20{,}345 entries). Pre-filtering to shared genes yielded zero overlap due to gene-name format mismatch. These failures reflect a standardized-evaluation limitation, not model quality: both models function correctly on data formatted to their specifications.

\paragraph{Expanded audit.} \textcolor{red}{[PENDING: scIB metrics on the contenders-6 set. Inversion rates, correlations, and per-metric results to be added after the expanded audit completes.]}
