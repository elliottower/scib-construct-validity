...
$d \geq 512$, none. A within-panel 2$\times$2 comparison confirms
dimensionality is causal: scVI (foundation model, $d = 50$) clears the
null in 16/25 tissues; bag-of-genes PCA ($d = 512$) fails in all 25.
...
$k$-dependent signal at low $d$ where CKA has genuine discriminative
power. Bio-conservation metrics show a third instance of the same
dissociation: scores increase at intermediate noise levels before
collapsing under controlled degradation (single-seed; replication
pending). We propose a usability criterion: given $k$ shared cell types
and dimension $d$, compute $\mathbb{E}[\mathrm{CKA}]$; if the expected
effect size does not exceed it, CKA is uninformative as an absolute
measure. Every current foundation model operates in the regime where
...
Four findings emerge. First, most metrics rank well: Procrustes
...
null; at $d \geq 512$, none do. The boundary falls inside the range of
dimensions that current foundation models use. Third, two attempted
repairs fail in diagnosable ways: a pre-registered spectral-gap
statistic inverts (null baselines more concentrated than trained
models), and $z$-score normalization degrades ranking by removing real
$k$-dependent signal. Fourth, bio-conservation metrics show preliminary
evidence of non-monotonic response to controlled embedding degradation,
with scores increasing at intermediate noise levels before collapsing
(single-seed; multi-seed replication planned).
...
The non-obvious claim is that every current single-cell foundation model
operates in the regime where this criterion is violated, while the
PCA-based methods the field used previously were on the correct side of
the boundary.
...
\textcolor{red}{[PCA-reduction robustness check: Geneformer embeddings
PCA-reduced from $d = 512$ to $d = 50$ and $d = 200$, CKA recomputed
against the $(k, d_{\mathrm{reduced}})$ null. If the reduced embeddings
clear the null, dimensionality is confirmed as causal independent of
model architecture. Results pending.]}
...
models ($d \geq 512$) crossed to the wrong side. A researcher using CKA
to evaluate scVI ($d = 50$) gets real certification signal; the same
researcher using the same metric on Geneformer ($d = 512$) gets none,
and the metric gives no warning that it has failed.
...
increase statistical power. The $d = 50$ discriminability evidence
relies on scVI as the sole foundation model at that dimensionality; a
PCA-reduction control (Geneformer at $d = 512$ projected to $d = 50$
and $d = 200$) is in progress to confirm that dimensionality is causal
independent of model architecture. The noise dose-response experiment
...
distinguish genuine non-monotonicity from clustering jitter. The PBMC ceiling effect limits
